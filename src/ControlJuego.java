...
public class ControlJuego {
	...
	public void inicializarPartida() {
		...
		for (int i = 0; i < MINAS_INICIALES; i++) {
			do {
				aleatorio1 = generarAleatorio();
				aleatorio2 = generarAleatorio();

			} while (tablero[aleatorio1][aleatorio2] == MINA);
	...
	private int calculoMinasAdjuntas(int i, int j) {
		...
		for (filas = Math.max(i-1,0); filas <= Math.min( i + 1,LADO_TABLERO-1); filas++) {
			for (columnas = Math.max(j - 1,0); columnas <= Math.min(j + 1,LADO_TABLERO-1); columnas++) {
